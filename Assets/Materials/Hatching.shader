...
Shader "Custom/HatchingComposite"
{
    Properties
    {
        ...
        _Power("Tiling", Float) = 1
        _Minus("Tiling", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed intensity = 1 - pow(dot(diffuse, fixed3(0.2326, 0.7152, 0.0722)), _Power) - _Minus;

                fixed3 col =  Hatching(i.uv * 8 * _Tiling, intensity);
                intensity = dot(col, fixed3(0.2326, 0.7152, 0.0722));

                color = lerp(_Color, _Color2, intensity);
